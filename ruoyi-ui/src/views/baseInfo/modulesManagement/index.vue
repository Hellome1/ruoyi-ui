...
<script>
import { getList as getModuleList, setList as setModuleList, addList as addModuleList, deleteList as deleteModuleList, switchAppling } from '@/apir/common';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getModuleList('menu/dict', Object.assign(param, {
        pageNumber: this.queryParams.pageNum,
        pageSize: this.queryParams.pageSize
      })).then(res => {
        // console.log({...res});
        this.rawData = this.copy(res.data, true); // 储存原始数据供修改表单显示
        this.moduleList = this.handleNullInData(res.data);
        this.total = res.recordsTotal;
        this.loading = false;
      }).catch((err) => {
        this.loading = false;
        this.total = 0;
        if (err.recordsTotal === 0) this.moduleList = [];
      });
    },
    // 修改列表数据的null字段
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.title === "修改"){
            // 修改
            setModuleList('menu/dict/update', this.form).then(res => {
              if (res.code === 200) this.responseSuccess(res);
              else this.responseErr(res);
            });
          } else {
            // 新增
            this.addNeedIdk && (this.form[this.idk] = "");
            this.formCheckout();
            addModuleList('menu/dict', this.form).then(res => {
              if (res.code === 200) this.responseSuccess(res);
              else this.responseErr(res);
            });
          }
    ...
    handleDelete(row) {
      ...
      let param = {};
      param['ids'] = row[this.idk] || ids.join('|');
      this.$modal.confirm('是否确认删除编号为"' + roleIds + '"的数据？')
      .then(function() {
        return deleteModuleList('menu/dict/delete', param);
      }).then(res => {
        if (res.code === 200) this.responseSuccess(res);
        else this.responseErr(res);
      }).catch(() => {});
